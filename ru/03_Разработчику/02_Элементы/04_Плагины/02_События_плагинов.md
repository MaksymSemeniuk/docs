...
### OnBeforeParseParams ###
Запускается перед парсингом параметров в вызове сніпета.
```
manager/includes/document.parser.class.inc.php
```
...
type - тип элемента
args - массив в формате Ім'я параметру => значение
```
...
```
Может быть вызвано разными сніпетами авторизации. НаПриклад WebLogin:
...
```
Используется для изменения различных свойств документа после создания дубликата (наПриклад можно сделать счетчик количества дублей)
...
```
Можно использовать, наПриклад, для автоматического, везвозвратного удаления документов из таблицы (при это не стоит забывать, что нужно удалять и ТВ параметры связанные с документами).
...
Событие возникающие при определении шаблона документа. Может использоваться при смене шаблона документа (наПриклад подтягивать специфические свойства)
```
manager/actions/mutate_content.dynamic.php
```
**Доступные переменные:**
...
**Доступные переменные:**
```
$editor - Назва активного редактора из OnRichTextEditorRegister
