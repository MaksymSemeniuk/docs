...
сніпет выводит корзину заказов двух типов (расширенный и простой). 

сніпет можно вызывать на странице несколько раз, но корзина выводится только в месте первого вызова. Второй и последующие вызовы игнорируются. 

**Приклад:**
*Если корзина на страницах каталога товаров находится в правой колонке шаблона, а на странице оформления заказа вызов сніпета вставлен в контент страницы, то на этой странице корзина будет выведена только в ценральной колонке (в правой вызов сніпета можно не убирать).*
...
<tr>
  ...
  <td>price</td>
  <td>Если ваш TV для цены имеет Назва не "price", <b>обязательно</b> укажите это имя в данном параметре.</td>
</tr>
<tr>
...
<tr>
  ...
  <td>если на сайте используется другая JS-библиотека (не JQuery), наПриклад Mootools, Prototype.</td>
  <td>0</td>
  <td>&nbsp;</td>
</tr>
<tr>
...
<tr>
  ...
  <td>Чанк содержит сразу 3 шаблона - для пустой, заполненной расширенной и простой корзины.<br> Если чанки хранятся в файлах, можно указать путь с командой "@FILE:". Если чанки созданы в системе управления, то указывается только имя чанка (наПриклад &cartTpl=`shopCart`)</td>
</tr>
<tr>
...
<tr>
  ...
  <td>0</td>
  <td>Обычно используется в вызове сніпета краткого вида корзины. Указывается ID страницы с подробным видом корзины (оформление заказа). На этой странице краткий вид корзины будет отключен.</td>
</tr>
</tbody></table>
...
<tr>
  ...
  <td>
  <b>[+inner+]</b> - рядок с информацией о товаре (cartRowTpl);<br>
...
<tr>
  ...
  <b>[+addit_data+]</b> - рядок с информацией о доп. параметрах товара;<br>
  <b>[+price+]</b> - цена;<br>
  ...
  <b>[+любой TV+]</b> - любой TV-параметр, наПриклад [+image+];<br>
  <b>[+shk_любой TV+]</b> - любой доп. параметр, выбранный при добавлении товара в корзину (из [+addit_data+]), наПриклад [+shk_param1+].
  </td>
  <td>Приклад: <b>chunk_shopCartRow.tpl</b> и <b>chunk_shopCartRow2.tpl</b></td>
</tr>
<tr>
  <td><b>&additDataTpl</b></td>
...
<tr>
  ...
  <td>Приклад: <b>chunk_shopOrderReport.tpl</b> и <b>chunk_shopOrderReportWebUser.tpl</b></td>
</tr>
<tr>
  <td>Страница, на которой вызывается сніпет Shopkeeper</td>
  <td></td>
  <td>
...
<tr>
  ...
  <td>Приклад <b>chunk_shopOrderForm.tpl</b> и <b>chunk_shopOrderFormWebUser.tpl</b></td>
</tr>
</tbody></table>
...
* OnSHKFrontendInit - инициализация сніпета Shopkeeper во внешней части.
...
* OnSHKmodPagePrint - вывод страницы модуля. Можно использовать для создания своей страницы (см. Приклад OnSHKmodPagePrint_plugin_example.txt).
...
В папке shopkeeper/docs/ есть Приклады плагинов.
...
## Приклады чанков в папке install/examples/ ##
...
* chunk_shopCartRow2.tpl - Приклад с картинкой (TV - image) и условием PHx.
* chunk_shopStuff.tpl - чанк краткой информации о товаре. Может быть использован как значение параметра &tpl при вызове Ditto.
* chunk_shopStuff_2.tpl - Приклад с двумя ценами для одного товара (продажа и прокат).
...
* chunk_FormSignup.tpl - Приклад чанка с шаблоном формы регистрации для сніпета WebSignup.
* chunk_weblogin.tpl - Приклад чанка с шаблоном формы авторизации сніпета WebLogin.
