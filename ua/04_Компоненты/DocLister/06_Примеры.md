...
```
После чего создаем JSON файл test.json в подпапке сніпета /config/custom/ с таким содержимым:
```
{
    "display" : "20",
    "summary" : "notags,len", 
...
Результат работы этого вызова будет идентичен предыдущему Прикладу.
...
После чего создаем php файл news.php с $\_lang массивом в подпапке сніпета /lang/
...
## Приклад вывода информации из таблицы, отличной от site_content

В данном Прикладе информация будет выводиться из таблицы site_snippets. В качестве колонки PrimaryKey выступает поле id. Сортировка происходит по возрастанию значений в колонке name.
