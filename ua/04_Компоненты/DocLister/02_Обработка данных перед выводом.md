...
```
Для того, чтоб получить значение параметров из вызова сніпета используйте конструкцию:

```
$parameter_value = $_DocLister->getCFGDef('parameter_name');
```
Данная конструкция позволяет получить значения параметра 'parameter_name' из вызова сніпета. 
